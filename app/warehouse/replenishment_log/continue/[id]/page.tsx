// app/warehouse/replenishment_log/continue/[id]/page.tsx

"use client";

import { useEffect, useState } from "react";
import { useParams, useRouter } from "next/navigation";
import LogNewReplenishmentForm from "@/components/add/replenishment/LogNewReplenishmentForm";
import { toast } from "sonner";
import { Skeleton } from "@/components/ui/skeleton";
import { Header } from "@/components/header";
import { DraftReplenishment } from "@/app/warehouse/replenishment_log/types/replenishment";

export default function ContinueReplenishmentPage() {
  const { id } = useParams(); // draft ID
  const router = useRouter();

  const [loading, setLoading] = useState(true);
  const [draftData, setDraftData] = useState<DraftReplenishment | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchDraft = async () => {
      try {
        ...
        const res = await fetch(`/api/replenishment/${id}`); // corrected endpoint
        if (!res.ok) throw new Error("Failed to fetch draft");
        const data = await res.json();

        console.log("Fetched draft replenishment:", data);

        setDraftData(data);
      } catch (err) {
        console.error(err);
        toast.error("Failed to load draft replenishment.");
        router.push("/warehouse/replenishment_log"); // fallback redirect
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div>
      ...
      <LogNewReplenishmentForm
        draftData={draftData}
        draftId={String(id)}
        onSaveSuccess={() => {
          toast.success("Draft updated successfully!");
          router.push("/warehouse/replenishment_log");
        }}
      />
    </div>
  );
