...
const WarehouseInventoryReportsPage = () => {
    ...
    const categoryStockData: ChartData[] = Object.values(
    filteredItems.reduce<Record<string, ChartData>>((acc, item) => {
      const catName = item.category?.name || "Uncategorized";
      if (!acc[catName]) acc[catName] = { name: catName, stock: 0 };
      acc[catName].stock += item.stock ?? 0;
      return acc;
    }, {})
  );

  const statusCountData: PieChartData[] = Object.values(
    filteredItems.reduce<Record<string, PieChartData>>((acc, item) => {
      const status = item.status || "Unknown";
    ...
    const COLORS = ["#0088FE", "#00C49F", "#FFBB28", "#FF8042", "#d12f2f"]
    ...
    return (
        ...
        <div className="h-screen w-full bg-[#ffedce] flex flex-col">
            <Header />
            <div className="p-6">
                <h1 className="text-3xl font-bold text-[#173f63] mb-4">Inventory Reports</h1>
                ...
                {loading ? (
                    ...
                    <div className="bg-white shadow rounded p-4 w-50">
                        <p className="text-lg">Total Items: <strong>{totalItems}</strong></p>
                        <p className="text-lg">Total Stocks: <strong>{totalStock}</strong></p>
                ...
                <div className="bg-white p-2 rounded shadow w-100 ml-2">
