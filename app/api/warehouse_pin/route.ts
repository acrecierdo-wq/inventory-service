...
//     if (!/^\d{4}$/.test(newPin)) {
//       return NextResponse.json({ error: "Enter new 4-digit PIN." }, { status: 400 });
//     }
...
//       // if record exists and has a pinhash = require to oldpin check
//       if (appUser && appUser.pinHash) {
//         if (!oldPin) {
//           return NextResponse.json({ error: "Current PIN is required to update." }, { status: 400 });
//         }
//         const validOld = await bcrypt.compare(oldPin, appUser.pinHash);
//         if (!validOld) {
...
//           .where(eq(appUsers.clerkId, user.id));
//       } else {
//         // no record or no pinhash = create new pin
//         const hash = await bcrypt.hash(newPin, 10);
//         const clerkEmail = user.emailAddresses[0]?.emailAddress || "";
//         const clerkName = user.fullName || user.firstName || "";
...
// app/customer/cus_dashboard/page.tsx
// "use client";
