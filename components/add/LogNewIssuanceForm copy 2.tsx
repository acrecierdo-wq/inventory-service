...
//   stock: number; // include stock from API
// };
...
//   // Update newItem state for debugging/UI
//   useEffect(() => {
//     setNewItem((prev) => ({
//       ...prev,
...
//   // Fetch combinations when item changes
//   useEffect(() => {
//     if (!selectedItem) {
//       setCombinations([]);
...
//   useEffect(() => setSelectedVariant(null), [selectedSize]);
//   useEffect(() => setSelectedUnit(null), [selectedVariant]);

//   // Auto-select single options
//   useEffect(() => {
//     if (!selectedItem) return;
//     if (availableSizes.length === 1 && !selectedSize) setSelectedSize(availableSizes[0]);
//     if (selectedSize && availableVariants.length === 1 && !selectedVariant) setSelectedVariant(availableVariants[0]);
//     if (selectedSize && selectedVariant && availableUnits.length === 1 && !selectedUnit) setSelectedUnit(availableUnits[0]);
//   }, [combinations]);
...
//   useEffect(() => {
//     if (selectedSize && !availableSizes.some((s) => String(s.id) === String(selectedSize.id))) setSelectedSize(null);
//     if (selectedVariant && !availableVariants.some((v) => String(v.id) === String(selectedVariant.id))) setSelectedVariant(null);
//     if (selectedUnit && !availableUnits.some((u) => String(u.id) === String(selectedUnit.id))) setSelectedUnit(null);
//   }, [availableSizes, availableVariants, availableUnits]);
...
//     if (availableVariants.length > 0 && !selectedVariant) return toast.error("Please select a variant.");
//     if (availableUnits.length > 0 && !selectedUnit) return toast.error("Please select a unit.");
//     if (!quantity || Number(quantity) <= 0 || isNaN(Number(quantity))) return toast.error("Enter a valid quantity.");
...
//       if (!found?.id) return toast.error("Failed to find matching item in inventory.");

//       const stock = itemStocks[found.id];
//       if (stock != null && Number(quantity) > stock) {
//         return toast.error(`Cannot add ${quantity} units. Only ${stock} in stock.`);
//       }
...
//       if (isDuplicate) return toast.error("This item with the selected options is already added.");
...
//         const res = await fetch(`/api/issuances/${draftId}`);
//         if (!res.ok) throw new Error("Failed to load draft.");
//         const draft = await res.json();

//         setInitialData(draft);
//         setClientName(draft.clientName || "");
//         setDispatcherName(draft.dispatcherName || "");
//         setCustomerPoNumber(draft.customerPoNumber || "");
//         setPrfNumber(draft.prfNumber || "");
//         setItems(
...
//       } catch (err) {
//         console.error(err);
//         toast.error("Could not load draft issuance.");
//       } finally {
