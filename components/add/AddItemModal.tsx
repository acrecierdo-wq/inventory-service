...
export default function AddItemModal({
  ...
  const handleDone = () => {
    ...
    if (!formData.name.trim())
      return toast.error("Please enter an item name.", { duration: 2000 });
    if (!formData.categoryId)
      return toast.error("Please select a category.", { duration: 2000 });
    if (!formData.unitId)
      return toast.error("Please select a unit.", { duration: 2000 });
    if (formData.reorderLevel === "")
      return toast.error("Please enter a reorder level.", { duration: 2000 });
    if (formData.criticalLevel === "")
      return toast.error("Please enter a critical level.", { duration: 2000 });
    if (formData.ceilingLevel === "")
      return toast.error("Please enter a ceiling level.", { duration: 2000 });
    if (formData.stock === "")
      return toast.error("Please enter a stock quantity.", { duration: 2000 });
    ...
    const isDuplicate = existingItems.some((item) => {
      ...
      const variantMatch =
        String(item.variantId ?? "") === String(formData.variantId ?? "");
      const sizeMatch =
        String(item.sizeId ?? "") === String(formData.sizeId ?? "");
      return (
        nameMatch && categoryMatch && unitMatch && variantMatch && sizeMatch
      );
    });
  ...
  return (
    <>
      ...
      <Dialog open={isOpen && !showSummary} onOpenChange={onClose}>
        <DialogContent className="w-[92vw] sm:w-[90vw] max-w-[500px] max-h-[85vh] sm:max-h-[90vh] overflow-y-auto p-0">
          <DialogHeader>
            <DialogTitle className="text-center text-white text-base sm:text-lg pt-2 sm:pt-3 bg-gradient-to-r from-[#e3ae01] via-[#fed795] to-[#fcf4d2] w-full h-[45px] sm:h-[50px] rounded-t-lg">
              <div className="text-[#173f63] font-bold">ADD NEW ITEM</div>
            </DialogTitle>

            <main className="w-full flex flex-col gap-0 mt-2 text-center text-base sm:text-lg py-3 sm:py-5 px-3 sm:px-4">
              <section className="flex flex-col sm:flex-row gap-2 sm:gap-9 mb-2">
                <p className="w-full sm:w-[120px] text-black text-start text-sm">
              ...
              <section className="flex flex-col sm:flex-row gap-2 sm:gap-9 mb-2">
                <div className="w-full sm:w-[120px] text-black text-start text-sm">
                  Category: <span className="text-red-500"> *</span>
                </div>
                <div className="w-full sm:w-[320px] text-black">
                  <Select
                ...
                <div className="w-full sm:w-[320px] text-black">
                  <Select
                    onValueChange={(value) =>
                      setFormData({
                        ...formData,
                        sizeId: value !== "" ? Number(value) : null,
                      })
                    }
                ...
                <div className="w-full sm:w-[320px] text-black">
                  <Select
                    onValueChange={(value) =>
                      setFormData({
                        ...formData,
                        variantId: value !== "" ? Number(value) : null,
                      })
                    }
                    value={
                      formData.variantId ? formData.variantId.toString() : ""
                    }
                  >
                    <SelectTrigger className="w-full px-2 py-1 border border-gray-200 rounded outline-none mb-2 hover:bg-gray-100 cursor-pointer">
                  ...
                  <input
                    ...
                    onPaste={(e) => {
                      ...
                      if (parsed < 0) {
                        ...
                        parsed = MAX_STOCK;
                        toast.error(`Stock cannot exceed ${MAX_STOCK}.`, {
                          duration: 2000,
                        });
                      }
                    ...
                    onChange={(e) => {
                      ...
                      if (parsed < 0) {
                        ...
                        parsed = MAX_STOCK;
                        toast.error(
                          `Stock quantity cannot exceed ${MAX_STOCK}`,
                          { duration: 2000 }
                        );
                      }
              ...
              <section className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 mr-0 sm:mr-5">
                <div>
              ...
              <div className="mt-4 flex justify-center gap-4">
                <button
                  ...
                  className={`h-7 w-20 bg-gray-300 text-sm text-gray-700 rounded hover:bg-gray-400 ${
                    isSubmitting
                ...
                <button
                  onClick={handleDone}
                  disabled={isSubmitting}
                  className={`h-7 w-20 text-white rounded text-sm ${
                    isSubmitting
                      ? "bg-gray-100 cursor-not-allowed"
                      : "bg-blue-400 hover:bg-blue-700 cursor-pointer"
                  }`}
                >
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-40 p-4">
          <div className="bg-white w-full max-w-[600px] p-4 sm:p-6 rounded shadow max-h-[90vh] overflow-y-auto">
            ...
            <div className="overflow-x-auto">
              <table className="w-full mt-4 mb-2 text-xs sm:text-sm border">
                <thead className="bg-[#f5e6d3] text-[#482b0e]">
                  <tr>
                    <th className="border px-1 sm:px-2 py-1">Item</th>
                    <th className="border px-1 sm:px-2 py-1">Category</th>
                    ...
                    <th className="border px-1 sm:px-2 py-1">Stock</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    ...
                    <td className="border px-1 sm:px-2 py-1">
                      {sizes.find((s) => s.id === formData.sizeId)?.name ||
                        "(None)"}
                    </td>
                    <td className="border px-1 sm:px-2 py-1">
                      {formData.stock || 0}
            ...
            <p className="mb-2 text-xs sm:text-sm text-gray-700">
              Reorder Level: {formData.reorderLevel ?? 0}
            </p>
            <p className="mb-2 text-xs sm:text-sm text-gray-700">
              Critical Level: {formData.criticalLevel ?? 0}
            </p>
            <p className="mb-2 text-xs sm:text-sm text-gray-700">
              Ceiling Level: {formData.ceilingLevel ?? 0}
            </p>

            <div className="flex justify-end gap-4 mt-6">
              <button
                onClick={() => setShowSummary(false)}
                className={`px-3 sm:px-4 py-2 text-sm bg-gray-300 rounded hover:bg-gray-400 ${
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className={`px-3 sm:px-4 py-2 text-sm bg-green-600 text-white rounded hover:bg-green-700 cursor-pointer ${
