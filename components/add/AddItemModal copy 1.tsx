...
export default function AddItemModal({
  ...
  useEffect(() => {
    ...
    if (formData.stock === 0) {
      status = "No Stock";
    } else if (formData.stock <= Number(formData.criticalLevel)) {
      status = "Critical Level";
    } else if (formData.stock  <= Number(formData.reorderLevel)) {
      status = "Reorder Level";
   } else if (formData.stock > Number(formData.ceilingLevel)) {
      status = "Overstock";
   }
      ...
      <DialogContent className="w-[800px]">
        ...
        <section className="flex flex-row gap-9">
          ...
          <div className="w-[320px] text-black">
            ...
            className="w-full text-sm px-2 py-1 border border-gray-200 rounded outline-none mb-2 hover:bg-gray-100 cursor-pointer"
            />
        ...
        <div className="mt-4 flex justify-center ">
          <Button onClick={handleSubmit}
            disabled={!formData.name || !formData.categoryId || !formData.unitId || formData.reorderLevel === "" || formData.criticalLevel === "" || formData.ceilingLevel === ""} 
