...
export default function AddSupplierModal({
  ...
  const handleDone = async () => {
    ...
    if (isDuplicate) {
      toast.error("This supplier already exists in the list.", {
        duration: 2000,
      });
      return;
    }
  ...
  return (
    <>
      ...
      <Dialog open={isOpen && !showSummary} onOpenChange={onClose}>
        <DialogContent className="w-[95vw] max-w-[800px] max-h-[90vh] overflow-y-auto overflow-x-hidden">
          <DialogHeader>
            <DialogTitle className="text-center text-white text-base sm:text-xl pt-3 bg-gradient-to-r from-[#e3ae01] via-[#fed795] to-[#fcf4d2] w-[calc(100%+52px)] h-[50px] -mt-[25px] -ml-[26px] rounded-t-lg">
              <div className="text-[#173f63] font-bold">ADD NEW SUPPLIER</div>
            </DialogTitle>

            {/* <DialogDescription className="text-center text-xl py-5"> */}
            <main className="w-full flex flex-col gap-0 mt-2 text-center text-base sm:text-xl py-3 sm:py-5 px-2 sm:px-0">
              <section className="flex flex-col sm:flex-row gap-2 sm:gap-9 mb-2">
                <p className="w-full sm:w-[120px] text-black text-start text-xs sm:text-sm">
              ...
              <div className="mt-4 flex justify-center gap-4 ">
                <button
                  type="button"
                  onClick={onClose}
                  className="h-8 sm:h-7 w-20 sm:w-15 bg-gray-300 text-xs sm:text-sm text-gray-700 rounded hover:bg-gray-400"
                >
                  Cancel
                </button>
                ...
                <button
                  ...
                  className={`h-8 sm:h-7 w-20 sm:w-15 text-white text-xs sm:text-sm rounded ${
                    isSubmitting
          ...
          <div className="bg-white w-full max-w-[600px] p-4 sm:p-6 rounded shadow max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-4 mt-6">
              <button
                ...
                className="px-3 sm:px-4 py-2 bg-gray-300 text-gray-700 text-xs sm:text-sm rounded hover:bg-gray-400 cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={handleSave}
                className="px-3 sm:px-4 py-2 bg-green-600 text-white text-xs sm:text-sm rounded hover:bg-green-700 cursor-pointer"
              >
                Save
              </button>
            </div>
