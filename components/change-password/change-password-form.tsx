...
export default function ChangePasswordForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mustChangePassword) {
        // forced flow: user must change password
        const res = await fetch("/api/users/change-password", {
            method: "POST",
            headers: { "Content-Type" : "application/json" },
            body: JSON.stringify({ newPassword }),
  ...
  return (
    <main className="flex flex-col items-center justify-center">
      <section className="bg-white w-[500px] rounded shadow p-5 mt-5">
        <div className="mx-auto mt-2 h-[20px] w-[450px] bg-gradient-to-r from-[#e3ae01] via-[#dc5034] to-[#faf0c5] rounded-2xl" />
        <h1 className="text-2xl font-bold text-center text-[#173f63] mt-5 mb-5">
          My Account
        </h1>
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 italic mb-1">
            * Must be at least 8 characters, include one uppercase, one number,
